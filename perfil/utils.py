...
def calcula_total(obj, campo):
    total = 0
    for o in obj:
        total += getattr(o, campo)
    return total


def calcula_equilibrio_financeiro():
    gastos_essenciais = Valores.objects.filter(data__month=datetime.now().month).filter(tipo='S').filter(categoria__essencial=False)
    gastos_nao_essenciais = Valores.objects.filter(data__month=datetime.now().month).filter(tipo='E').filter(categoria__essencial=False)
    ...
    print(total_gastos_essenciais, total_gastos_nao_essenciais)
